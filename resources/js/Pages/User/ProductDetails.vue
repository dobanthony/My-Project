...
// Main image reactive ref
const mainImage = ref(
  props.product.image ? `/storage/${props.product.image}` : 'https://via.placeholder.com/500x400?text=No+Image'
)

// Watch mainImage so the selected image is saved in the form
...
// Go to shop
const goToShop = () => {
  router.visit(`/shop/${props.product.shop.id}/products`)
}
</script>